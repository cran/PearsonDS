...
F21  <- function(aa,bb,cc,zz,tol=1e-8,minit,maxit,DEBUG=FALSE) {
  ...
  if (sum(ind)>0) {
    DDres <- .Call(paste("F21DD",suffix,sep=""),aa,bb,cc,zz[ind],minit,maxit,package="PearsonDS")
    res[ind] <- DDres$value
    ...
    if (sum(ind2)>0) {
      QDres   <- .Call(paste("F21QD",suffix,sep=""),aa,bb,cc,zz[ind[ind2]],minit,maxit,package="PearsonDS")
...
#  stopifnot((scale>0)&&(m>0.5))
  ...
  if (TRUE) {
    ...
    res[ind1] <- dpearsonIV(q[ind1],params=c(m,nu,location,scale)) *
                 ...
                 PearsonDS:::F21(1,m+nu/2*1i,2*m,2/(1-1i*x[ind1]),tol,...)

    res[ind2] <- 1-dpearsonIV(-q[ind2],params=c(m,-nu,-location,scale)) *
                 scale/(2*m-1) * (1i + x[ind2]) *
                 PearsonDS:::F21(1,m-nu/2*1i,2*m,2/(1+1i*x[ind2]),tol,...)
    if (abs(nu)<(4-2*sqrt(3))*m) {
      res[ind3] <- (1-exp(-(nu+1i*2*m)*pi))^(-1) -
                   ...
                   PearsonDS:::F21_3_5(1,2-2*m,2-m+1i*nu/2,(1+1i*x[ind3])/2,
                                       tol,...)
      res[ind4] <- 1 - (1-exp(-(-nu+1i*2*m)*pi))^(-1) +
                   (1i*scale*dpearsonIV(-q[ind4],params=c(m,-nu,-location,scale)
                   ))/(1i*(-nu)-2*m+2) * (1+(-x[ind4])^2) *
                   PearsonDS:::F21_3_5(1,2-2*m,2-m-1i*nu/2,(1-1i*x[ind4])/2,
                                       tol,...)
    } else {
      res[ind3] <- (1-exp(-(nu+1i*2*m)*pi))^(-1) -
                   (1i*scale*dpearsonIV(q[ind3],params=c(m,nu,location,scale)))/
                   (1i*nu-2*m+2) * (1+x[ind3]^2) *
                   PearsonDS:::F21(1,2-2*m,2-m+1i*nu/2,(1+1i*x[ind3])/2,tol,...)
      res[ind4] <- 1 - (1-exp(-(-nu+1i*2*m)*pi))^(-1) +
                   (1i*scale*dpearsonIV(-q[ind4],params=c(m,-nu,-location,scale)
                   ))/(1i*(-nu)-2*m+2) * (1+(-x[ind4])^2) *
                   PearsonDS:::F21(1,2-2*m,2-m-1i*nu/2,(1-1i*x[ind4])/2,tol,...)
    }
    res <- Re(res)
  }
}
...
#  stopifnot((scale>0)&&(m>0.5))
  ...
  Re((exp(-nu*phi)*exp((2-2*m)*phi*1i)*
     (PearsonDS:::F21(2-2*m,u,u+1,exp(phi*2i),DEBUG=DEBUG,tol=tol)/
     norm)-1)/
